...
public interface IGrammar {
   ...
   * @param timeLimit duration after which tokenization is aborted, in which case the returned
   *     result will have {@link ITokenizeLineResult#isStoppedEarly()} set to <code>true</code>
   * @return the result of the tokenization.
   */
  ITokenizeLineResult<IToken[]> tokenizeLine(
      String lineText, @Nullable IStateStack prevState, @Nullable Duration timeLimit);

  /**
   * Tokenize `lineText` using previous line state `prevState`. The result contains the tokens in
   * binary format, resolved with the following information:
   *
   * <pre>
   * - language
   ...
   * </pre>
   *
   * e.g. for getting the languageId: <code>
   * (token & EncodedTokenDataConsts.LANGUAGEID_MASK) >>> EncodedTokenDataConsts.LANGUAGEID_OFFSET
   * </code>
   */
   ...
   *     result will have {@link ITokenizeLineResult#isStoppedEarly()} set to <code>true</code>
   */
  ITokenizeLineResult<int[]> tokenizeLine2(
      String lineText, @Nullable IStateStack prevState, @Nullable Duration timeLimit);
}
