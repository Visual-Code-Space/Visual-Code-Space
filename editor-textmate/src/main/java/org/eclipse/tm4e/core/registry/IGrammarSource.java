...
public interface IGrammarSource {
  ...
  static IGrammarSource fromFile(
      final File file, @Nullable final ContentType contentType, @Nullable final Charset charset) {
      ...
      public Reader getReader() throws IOException {
        return new BufferedReader(
            new InputStreamReader(
                new FileInputStream(file), charset == null ? StandardCharsets.UTF_8 : charset));
      }
  ...
  static IGrammarSource fromInputStream(
      InputStream stream, @Nullable final String fileName, @Nullable final Charset charset) {

    final var contentType1 = guessFileFormat(fileName);

    try (var reader =
        new BufferedReader(
            new InputStreamReader(stream, charset == null ? StandardCharsets.UTF_8 : charset))) {
   ...
   * @throws IllegalArgumentException if the content type is unsupported or cannot be determined
   */
  static IGrammarSource fromResource(
      final Class<?> clazz,
