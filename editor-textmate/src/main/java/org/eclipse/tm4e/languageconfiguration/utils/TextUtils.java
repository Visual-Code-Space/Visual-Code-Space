...
public class TextUtils {
  ...
  public static String normalizeIndentation(
      final String str, final int tabSize, final boolean insertSpaces) {
    int firstNonWhitespaceIndex = TextUtils.firstNonWhitespaceIndex(str);
    if (firstNonWhitespaceIndex == -1) {
      firstNonWhitespaceIndex = str.length();
    }
    return TextUtils.normalizeIndentationFromWhitespace(
            str.substring(0, firstNonWhitespaceIndex), tabSize, insertSpaces)
        + str.substring(firstNonWhitespaceIndex);
  }

  private static String normalizeIndentationFromWhitespace(
      final String str, final int tabSize, final boolean insertSpaces) {
    int spacesCnt = 0;
  ...
  public static String getIndentationFromWhitespace(
      final String whitespace, final int tabSize, final boolean useTab) {

    final String tab = useTab ? "\t" : " ".repeat(tabSize);
    int indentOffset = 0;

    while (whitespace.startsWith(tab, indentOffset)) {
  ...
  public static String getLinePrefixingWhitespaceAtPosition(
      final Content d, final CharPosition position) {

    var line = d.getLine(position.line);

    var startIndex =
        IntPair.getFirst(
            io.github.rosemoe.sora.text.TextUtils.findLeadingAndTrailingWhitespacePos(line));

    return line.subSequence(0, startIndex).toString();
  }

  /**
   * Returns the leading whitespace of the string. If the string contains only whitespaces, returns
   * entire string
   */
