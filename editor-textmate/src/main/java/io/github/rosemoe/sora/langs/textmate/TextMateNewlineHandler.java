...
public final class TextMateNewlineHandler implements NewlineHandler {
    ...
    switch (enterAction.indentAction) {
      ...
      case Outdent:
        final var indentation =
            TextUtils.getIndentationFromWhitespace(enterAction.indentation, language.getTabSize(), language.useTab());
  ...
  protected Pair<String, String> getIndentForEnter(Content text, CharPosition position) {
    ...
    var afterEnterAction =
        getInheritIndentForLine(
            new WrapperContentImp(text, position.line, beforeEnterText), true, position.line + 1);
    ...
    var indent = language.useTab() ? "\n" : " ".repeat(language.getTabSize());
   ...
   */
  @Nullable
  private InheritIndentResult getInheritIndentForLine(
      WrapperContent wrapperContent, boolean honorIntentialIndent, int line) {
