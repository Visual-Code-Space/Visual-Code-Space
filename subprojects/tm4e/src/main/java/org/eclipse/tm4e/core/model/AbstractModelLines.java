 ...
 *
 * <p>Implementation class must synchronize lines with the lines of the editor content when it
 * changed.
 */
public abstract class AbstractModelLines {
  ...
  static final class ModelLine {
    /**
     * specifies if the {@link ModelLine#tokens} and/or the {@link ModelLine#startState} are
     * out-of-date and do not represent the current content of the related line in the editor
     */
  ...
  @Nullable private TMModel model;
   ...
   * @throws IndexOutOfBoundsException if <code>
   *     lineIndex < 0 || lineIndex >= {@link #getNumberOfLines()}</code>
   */
  ...
  protected void replaceLines(final int lineIndex, int linesRemoved, final int linesAdded) {
    if (linesRemoved == 0 && linesAdded == 0) return;
  ...
  ModelLine getOrNull(final int lineIndex) {
    synchronized (list) {
      if (lineIndex > -1 && lineIndex < list.size()) return list.get(lineIndex);
    }
    return null;
  }
          ...
          sb -> {
            if (!list.isEmpty()) {
              for (int i = 0; i < list.size(); i++) {
                sb.append(i).append(": isInvalid=").append(list.get(i).isInvalid).append(", ");
              }
