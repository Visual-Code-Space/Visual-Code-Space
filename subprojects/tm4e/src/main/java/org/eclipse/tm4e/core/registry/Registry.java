...
public final class Registry {
  ...
  private static final Logger LOGGER = Logger.Companion.newInstance(Registry.class.getName());
  ...
  public Registry(final IRegistryOptions options) {
    ...
    this._syncRegistry =
        new SyncRegistry(Theme.createFromRawTheme(options.getTheme(), options.getColorMap()));
  }
  ...
  public void setTheme(final IThemeSource source) throws TMException {
    try {
      this._syncRegistry.setTheme(
          Theme.createFromRawTheme(ThemeReader.readTheme(source), _options.getColorMap()));
    } catch (final Exception ex) {
      throw new TMException(
          "Loading theme from '" + source.getFilePath() + "' failed: " + ex.getMessage(), ex);
    }
  }
  ...
  /**
   * Load the grammar for `scopeName` and all referenced included grammars asynchronously. Please do
   * not use language id 0.
   */
  @Nullable
  ...
  private IGrammar _loadGrammar(
    ...
    final var dependencyProcessor =
        new ScopeDependencyProcessor(this._syncRegistry, initialScopeName);
    ...
    return this._grammarForScopeName(
        initialScopeName, initialLanguage, embeddedLanguages, tokenTypes, balancedBracketSelectors);
  }
