...
class AceCodeEditor : WebView {
  ...
  fun applyComposition(text: Any?, composition: Any?) = loadJs("editor.applyComposition($text, $composition)")
    ...
    val optionsString =
      options?.let {
        ...
        with(it) {
          ...
          // TODO:
          // start?.let {
          //   parts += "start: {start: {row: ${it.start.row}, column: ${it.start.column}}, end: {row: ${it.end.row}, column: ${it.end.column}}}"
          // }
          // range?.let {
          //   parts += "range: {start: {row: ${it.start.row}, column: ${it.start.column}}, end: {row: ${it.end.row}, column: ${it.end.column}}}"
          // }
  ...
  fun jumpToMatching(select: Boolean, expand: Boolean) = loadJs("editor.jumpToMatching($select, $expand")
